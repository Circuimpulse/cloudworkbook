    ...
    <div className="mt-6 space-y-4">
      {groups.map((group, groupIndex) => {
        ...
        return (
          <div
            key={rangeLabel}
            className="overflow-hidden rounded-2xl border border-slate-200 bg-white/60 shadow-sm backdrop-blur-xl transition-all hover:bg-white"
          >
            <button
              type="button"
            ...
            >
              <div className="px-8 py-8">
                ...
                <p className="mt-2 pl-10 text-xl font-medium text-slate-500 md:text-2xl">
                  {group.length}セクション
                </p>
              </div>
            </button>
                ...
                <div className="space-y-3">
                  {group.map((section) => (
                    <div key={section.id}>
                      ...
                      <div className="mt-2 h-px w-full max-w-[300px] bg-slate-200" />
                    </div>
                  ))}
                </div>
              </div>
