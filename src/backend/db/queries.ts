...
/**
 * 大分類（level=1）のカテゴリ一覧を取得
 */
export async function getTopLevelCategories() {
  return db
    .select()
    .from(ipaCategories)
    .where(eq(ipaCategories.level, 1))
    .orderBy(ipaCategories.name)
    .all();
}

/**
 * 指定した親カテゴリの子カテゴリ一覧を取得
 */
export async function getChildCategories(parentId: number) {
  return db
    .select()
    .from(ipaCategories)
    .where(sql`${ipaCategories.parentId} = ${parentId}`)
    .orderBy(ipaCategories.name)
    .all();
}
...
/**
 * カテゴリ全件取得（階層構造付き）
 * 大分類→中分類→小分類の順でフラットに返す
 */
export async function getAllCategoriesFlat() {
  return db
    .select()
    .from(ipaCategories)
    .orderBy(ipaCategories.level, ipaCategories.name)
    .all();
}

/**
 * カテゴリIDで問題一覧を取得（カテゴリとその子孫を含む）
 */
export async function getQuestionsByCategoryId(categoryId: number) {
  return db
    .select({
      question: questions,
      section: sections,
      category: ipaCategories,
    })
    .from(questions)
    .innerJoin(sections, eq(questions.sectionId, sections.id))
    .leftJoin(ipaCategories, eq(questions.categoryId, ipaCategories.id))
    .where(sql`${questions.categoryId} = ${categoryId}`)
    .orderBy(questions.examYearId, questions.questionNumber)
    .all();
}
...
/**
 * 試験区分に紐づく年度一覧を取得（新しい順）
 */
export async function getExamYearsByExamId(examId: number) {
  return db
    .select()
    .from(examYears)
    .where(eq(examYears.examId, examId))
    .orderBy(desc(examYears.year), examYears.season)
    .all();
}
 ...
 * 全試験年度を取得（試験区分情報付き）
 */
export async function getAllExamYears() {
  return db
    .select({
      examYear: examYears,
      exam: exams,
    ...
    .from(examYears)
    .leftJoin(exams, eq(examYears.examId, exams.id))
    .orderBy(desc(examYears.year), examYears.season)
    .all();
}

/**
 * 特定の年度・回の問題一覧を取得（問番号順）
 */
export async function getQuestionsByExamYear(examYearId: number) {
  return db
    .select({
      question: questions,
      section: sections,
    ...
    .innerJoin(sections, eq(questions.sectionId, sections.id))
    .leftJoin(ipaCategories, eq(questions.categoryId, ipaCategories.id))
    .where(eq(questions.examYearId, examYearId))
    .orderBy(questions.questionNumber)
    .all();
}

/**
 * 図表あり問題を取得（imageUrl未設定のものを優先）
 * 画像URLを手動設定する際の管理用
 */
export async function getQuestionsWithImageFlag(examId?: number) {
  if (examId) {
    return db
      .select({
        question: questions,
      ...
      .from(questions)
      .innerJoin(sections, eq(questions.sectionId, sections.id))
      .where(and(eq(questions.hasImage, true), eq(sections.examId, examId)))
      .orderBy(questions.examYearId, questions.questionNumber)
      .all();
  }
  return db
